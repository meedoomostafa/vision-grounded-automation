...
def _fill_save_path_with_pywinauto(absolute_path: str) -> bool:
    """Try setting the Save As path directly via dialog edit controls."""
    try:
    ...
    for backend in ("uia", "win32"):
        try:
            ...
            # In common file dialogs, the filename box is usually the lowest edit control.
...
def _submit_save_via_directory_navigation(file_path: Path) -> None:
    """Fallback for dialogs that reject absolute paths in the file name box."""
    output_dir = str(file_path.parent.resolve())
    file_name = file_path.name

    # Focus the address bar, navigate to target directory, then provide filename.
    hotkey("alt", "d")
    wait_ms(120)
...
def force_terminate_notepad_processes() -> None:
    """Force-close all Notepad instances to guarantee clean desktop state."""
    if config.DRY_RUN:
