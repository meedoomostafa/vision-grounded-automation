...
# Consistent color scheme for annotations
COLOR_REGION = (255, 255, 0)       # yellow — candidate regions
COLOR_CROSSHAIR = (255, 0, 0)     # red — final detection
...
def annotate_detection(
    ...
    """Draw detection overlay on a screenshot.

    Args:
        image: Original screenshot.
        coords: (x, y) final detection coordinates.
        regions: Optional list of region dicts with x1,y1,x2,y2 keys.
        label: Text label for the detection point.
...
def save_annotated(image: Image.Image, name: str) -> None:
    """Save annotated image to the screenshots directory."""
...
def generate_som_overlay(image: Image.Image) -> tuple[Image.Image, dict[int, tuple[int, int]]]:
    """Generates a Set-of-Mark (SoM) overlay using OpenCV contour detection.
    
    Draws bounding boxes with unique numeric IDs over distinct UI elements.
    
    Returns:
        Tuple of (Annotated PIL Image, Dictionary mapping ID -> (center_x, center_y))
    """
    # Avoid layout strides issues with PIL -> OpenCV conversion
    ...
    # Morphological closing to group icon borders with their nested designs
    ...
    for cnt in contours:
        ...
        if w < 12 or h < 12: # Ignore tiny random edges
            continue
        if w > width * 0.9 or h > height * 0.9: # Ignore whole screen borders
            continue
