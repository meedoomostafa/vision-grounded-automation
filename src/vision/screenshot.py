...
def capture_screen(monitor_index: int = 1) -> Image.Image:
    """Capture full screen via mss, returns PIL.Image in RGB mode.
...
def crop_region(image: Image.Image, bbox: tuple[int, int, int, int]) -> Image.Image:
    """Crop sub-region from image with bounds clamping.
    ...
    Returns:
        Cropped PIL.Image. Returns 1x1 pixel image if bbox is degenerate.
    """
    x1, y1, x2, y2 = bbox
    w, h = image.size
