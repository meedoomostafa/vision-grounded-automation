...
def retry(max_attempts: int = 3, backoff_base: float = 2.0, exceptions: tuple = (Exception,)):
    ...
    Retries the wrapped function up to max_attempts times on specified
    exception types. Delay doubles each attempt with random jitter
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if attempt < max_attempts:
                        # Exponential backoff with uniform jitter [0, 1)
